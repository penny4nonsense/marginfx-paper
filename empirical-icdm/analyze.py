...
from icdm_bootstrap import bootstrap_cell
...
# Inner thread count for the tree learners. The base fit wants every core;
# a bootstrap replicate does not, because the bootstrap is already running
# many replicates in parallel and nested -1 would oversubscribe the machine
# by a factor of the worker count. Workers set MFX_INNER_JOBS=1.
_INNER_JOBS = int(os.environ.get('MFX_INNER_JOBS', '-1'))
    ...
    if model_name in ('logistic', 'linear'):
        ...
        if outcome_type == 'classification':
            model = xgb.XGBClassifier(
                ...
                verbosity=0, eval_metric='logloss', n_jobs=_INNER_JOBS,
            )
        else:
            model = xgb.XGBRegressor(
                n_estimators=200, max_depth=4,
                learning_rate=0.05, random_state=seed,
                verbosity=0, n_jobs=_INNER_JOBS,
            )
        model.fit(X, y)
        return model
    ...
    for spec_name, features in specifications.items():
        ...
        for model_name in model_names:
            ...
            try:
                ...
                # --- AMEs ---
                # Refits every replicate through fit_model, the same function
                # that built `model` above. The published tables came from a
                # warm-start path whose replicates differed from the base fit
                # -- forests not refit at all, XGBoost at double the boosting
                # rounds, the network at 10 epochs instead of early-stopped
                # 500, and the regression network stripped of its rescaling
                # wrapper. Rebuilding through fit_model removes all four by
                # construction.
                tidy = bootstrap_cell(
                    model, model_name, X, y, outcome_type,
