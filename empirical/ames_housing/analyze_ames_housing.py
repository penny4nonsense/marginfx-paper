...
Key difference from Adult and Credit Default:
    This is a REGRESSION problem — outcome is SalePrice in dollars.
    AMEs are interpreted as dollars per unit change in each feature.
    Models used: linear regression instead of logistic regression.

Results saved to empirical/ames_housing/results/.
...
# Linear regression instead of logistic for regression outcome
MODELS = ['linear', 'rf', 'xgboost', 'tensorflow']

PROCESSED_PATH = os.path.join(
    PAPER_DIR, 'data', 'processed', 'ames_housing',
    'ames_housing_processed.csv'
)
...
if __name__ == '__main__':
    print("=" * 60)
    print("marginfx Empirical Analysis — Ames Housing")
    print("=" * 60)
    print(f"\nSettings:")
    ...
    if not ame_results.empty:
        print("\nAME Results Preview (full spec, linear regression):")
        preview = ame_results[
            (ame_results['model'] == 'linear') &
            (ame_results['spec'] == 'ABC')
        ][['term', 'estimate', 'std_error', 'p_value']].round(2)
        print(preview.to_string(index=False))
        print("\n(AMEs in dollars per unit change in each feature)")
